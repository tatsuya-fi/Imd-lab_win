...
CKinectFusionColorBasics::CKinectFusionColorBasics() :
	...
	m_depthImageResolution(IMAGE_RESOLUTION),
	m_colorImageResolution(IMAGE_RESOLUTION),
    ...
    m_cColorIntegrationInterval(2)  // Capturing color has an associated processing cost, so we do not capture every frame here
{
	...
	m_reconstructionParams.voxelCountX = 1024.
		;   // 512 / 256vpm = 2m wide reconstruction
	m_reconstructionParams.voxelCountY = 768;   // Memory = 512*384*512 * 4bytes per voxel
...
void CKinectFusionColorBasics::initDepthMat()
{
	depthMat.setINuiSensor(m_pNuiSensor);
	depthMat.setImageSize(m_colorImageResolution);
...
void CKinectFusionColorBasics::ProcessDepth()
{
    ...
    else
    {
		...
		////////
		// Transform Nui color image frame to Mat image
		depthMat.setRgbImage(rgbImage, imageFrame);
		// デバッグ用テキスト表示
		cv::putText(rgbImage, "Click count:", cv::Point(20, 25), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 0, 200), 2, CV_AA);
		switch (clickCount)
		{
		case 0:
			cv::putText(rgbImage, "0", cv::Point(150, 25), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 0, 200), 2, CV_AA);
			break;
		case 1:
			cv::putText(rgbImage, "1", cv::Point(150, 25), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 0, 200), 2, CV_AA);
			break;
		case 2:
			cv::putText(rgbImage, "2", cv::Point(150, 25), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 0, 200), 2, CV_AA);
			break;
		case 3:
			cv::putText(rgbImage, "3", cv::Point(150, 25), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 0, 200), 2, CV_AA);
			break;
		case 4:
		case 5:
			cv::putText(rgbImage, "4 Done.", cv::Point(150, 25), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 0, 200), 2, CV_AA);
			break;
		default:
			cv::putText(rgbImage, "error", cv::Point(150, 25), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 0, 200), 2, CV_AA);

			break;
		}
		cv::putText(rgbImage, "Cap count:", cv::Point(20, 50), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 0, 200), 2, CV_AA);
		switch (capCount)
		{
		case 0:
			cv::putText(rgbImage, "0", cv::Point(150, 50), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 0, 200), 2, CV_AA);
			break;
		case 1:
			cv::putText(rgbImage, "1", cv::Point(150, 50), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 0, 200), 2, CV_AA);
			break;
		case 2:
			cv::putText(rgbImage, "2 Done.", cv::Point(150, 50), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 0, 200), 2, CV_AA);
			break;
		default:
			cv::putText(rgbImage, "error", cv::Point(150, 50), cv::FONT_HERSHEY_SIMPLEX, 0.7, cv::Scalar(0, 0, 200), 2, CV_AA);
		...
		// キャリブレーション用の画像表示
		const int wWidth = 1680 - 16, wHeight = 1050 - 80;
		const int dis = 40, r = 20;
		cv::Mat calibImg = cv::Mat::zeros(wHeight, wWidth, CV_8UC3);
		cv::circle(calibImg, cv::Point2d(dis, dis), r, CV_RGB(0, 0, 255), -1);
		cv::circle(calibImg, cv::Point2d(wWidth-dis, dis), r, CV_RGB(255, 0, 0), -1);
		cv::circle(calibImg, cv::Point2d(dis, wHeight-dis), r, CV_RGB(255, 0, 0), -1);
		cv::circle(calibImg, cv::Point2d(wWidth-dis, wHeight-dis), r, CV_RGB(255, 0, 0), -1);
		//cv::namedWindow("calibImg", CV_WINDOW_FREERATIO);
		cv::imshow("calibImg", calibImg);
    ...
    // Test to see if camera tracking failed. GetCurrentWorldToCameraTransform
    ...
    else
    {
        ...
        if (SUCCEEDED(hr))
        {
			...
			// Wait the key input for calibration
			// 終了処理 閉じるボタンだとフリーズする可能性がある（原因不明）
			...
			case KEY_ESC:
				exit(0);
				break;
			case KEY_SPACE:	// スペースキー
			case 'c':
				if (capCount == 0 || capCount == 1)
				{
					depthMat.setMat(m_worldToCameraTransform, capCount);
					capCount++;
					cv::waitKey(20);
				}
				if (capCount == 2)
				{
					depthMat.caliculateCamToPrecamTransMat();
				}
				break;
			default:
				break;
			...
			if (capCount == 1) 
			{
				static Vector4 preCamPoint3d;
				depthMat.getPreCam3dPoint(m_worldToCameraTransform, preCamPoint3d);
				depthMat.drawPreCam(rgbImage, preCamPoint3d);
			}
			// キャリブレーション用の点を再投影する
			//if (clickCount == 5) 
			//{
			//	cv::Mat zerooo = cv::Mat::zeros(4, 1, CV_32F);
			//	zerooo.at<float>(3, 0) = 1;

			//	cv::Mat pointss = internalParam * TTT * zerooo;
			//	pointss = pointss.at<float>(2, 0);

			//	float disp3dTodisp2d = 40 / 0.003;

			//	int colorx = (int)(pointss.at<float>(0,0) * disp3dTodisp2d);
			//	int colory = (int)(pointss.at<float>(1,0) * disp3dTodisp2d);

			//	cv::circle(rgbImage, cv::Point2d(colorx, colory), 5, CV_RGB(0, 0, 255), -1);

			//	std::ofstream ofsa("debug.txt");
			//	ofsa << colorx << ", " << colory << std::endl;
			//}
        }
    }
...
void Mouse(int event, int x, int y, int flags, void*) // コールバック関数
{
	switch (event) {
	case CV_EVENT_LBUTTONUP:
		if (clickCount < 4)
		{
			...
			clickedPoints.at<float>(2 * clickCount, 0)		= (float)x - fx;
			clickedPoints.at<float>(2 * clickCount + 1, 0)  = (float)y - fy;

			A_cameraCarib.at<float>(2 * clickCount, 0) = caribPoints.at<float>(clickCount, 0) * f;
			A_cameraCarib.at<float>(2 * clickCount, 1) = caribPoints.at<float>(clickCount, 1) * f;
			A_cameraCarib.at<float>(2 * clickCount, 2) = f;
			A_cameraCarib.at<float>(2 * clickCount, 6) = (fx - x) * caribPoints.at<float>(clickCount, 0);
			A_cameraCarib.at<float>(2 * clickCount, 7) = (fx - x) * caribPoints.at<float>(clickCount, 1);

			A_cameraCarib.at<float>(2 * clickCount + 1, 3) = caribPoints.at<float>(clickCount, 0) * f;
			A_cameraCarib.at<float>(2 * clickCount + 1, 4) = caribPoints.at<float>(clickCount, 1) * f;
			A_cameraCarib.at<float>(2 * clickCount + 1, 5) = f;
			A_cameraCarib.at<float>(2 * clickCount + 1, 6) = (fy - y) * caribPoints.at<float>(clickCount, 0);
			A_cameraCarib.at<float>(2 * clickCount + 1, 7) = (fy - y) * caribPoints.at<float>(clickCount, 1);

			////////////////////////////////////
			/// solvePnP
			//points2d.push_back(cv::Point2f((float)x, (float)y));
			//points3d.push_back(cv::Point3f(caribPoints.at<float>(clickCount, 0), caribPoints.at<float>(clickCount, 1), 0));
		...
		if (clickCount == 4)
		{
			...
			cv::Mat matA = A_cameraCarib.t() * A_cameraCarib;
			cv::Mat cameraImgToDispTransform = matA.inv() * A_cameraCarib.t() * clickedPoints;

			// 回転の各成分を正規化
			float xDis = sqrt(pow(cameraImgToDispTransform.at<float>(0, 0), 2) + pow(cameraImgToDispTransform.at<float>(3, 0), 2) + pow(cameraImgToDispTransform.at<float>(6, 0), 2));
			cameraImgToDispTransform.at<float>(0, 0) /= xDis;
			cameraImgToDispTransform.at<float>(3, 0) /= xDis;
			cameraImgToDispTransform.at<float>(6, 0) /= xDis;
			float yDis = sqrt(pow(cameraImgToDispTransform.at<float>(1, 0), 2) + pow(cameraImgToDispTransform.at<float>(4, 0), 2) + pow(cameraImgToDispTransform.at<float>(7, 0), 2));
			cameraImgToDispTransform.at<float>(1, 0) /= yDis;
			cameraImgToDispTransform.at<float>(4, 0) /= yDis;
			cameraImgToDispTransform.at<float>(7, 0) /= yDis;

			//float a = cameraImgToDispTransform.at


			// 外積をとりZ成分の単位ベクトルを求める
			//float zx = cameraImgToDispTransform.at<float>(3, 0) * cameraImgToDispTransform.at<float>(7, 0) - cameraImgToDispTransform.at<float>(6, 0) * cameraImgToDispTransform.at<float>(4, 0);
			//float zy = cameraImgToDispTransform.at<float>(6, 0) * cameraImgToDispTransform.at<float>(1, 0) - cameraImgToDispTransform.at<float>(0, 0) * cameraImgToDispTransform.at<float>(7, 0);
			//float zz = cameraImgToDispTransform.at<float>(3, 0) * cameraImgToDispTransform.at<float>(0, 0) - cameraImgToDispTransform.at<float>(4, 0) * cameraImgToDispTransform.at<float>(1, 0);
			//float zDis = sqrt(zx*zx + zy*zy + zz*zz);
			//zx /= zDis;	   
			//zy /= zDis;    
			//zz /= zDis;
			cv::Mat xxx = (cv::Mat_<float>(3, 1) << cameraImgToDispTransform.at<float>(0, 0), cameraImgToDispTransform.at<float>(3, 0), cameraImgToDispTransform.at<float>(6, 0));
			cv::Mat yyy = (cv::Mat_<float>(3, 1) << cameraImgToDispTransform.at<float>(1, 0), cameraImgToDispTransform.at<float>(4, 0), cameraImgToDispTransform.at<float>(7, 0));
			cv::Mat zzz = xxx.cross(yyy);
			
			cv::Mat Tp_c = (cv::Mat_<float>(4, 4) <<
				cameraImgToDispTransform.at<float>(0, 0), cameraImgToDispTransform.at<float>(1, 0), zzz.at<float>(0,0), cameraImgToDispTransform.at<float>(2, 0) / xDis,
				cameraImgToDispTransform.at<float>(3, 0), cameraImgToDispTransform.at<float>(4, 0), zzz.at<float>(1,0), cameraImgToDispTransform.at<float>(5, 0) / xDis,
				cameraImgToDispTransform.at<float>(6, 0), cameraImgToDispTransform.at<float>(7, 0), zzz.at<float>(2,0), 1 / xDis,
				0, 0, 0, 1);

			TTT = Tp_c.clone();

			std::ofstream ofs("dispToCameraImgTransform.txt");
			ofs << TTT << std::endl;
