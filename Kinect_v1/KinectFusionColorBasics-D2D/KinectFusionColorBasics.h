...
const int calibPointsNum = 4;
static cv::Mat clickedPoints = cv::Mat::zeros(calibPointsNum * 2, 1, CV_32F);
static int clickCount;		// ウィンドウのクリック回数．四点
static cv::Mat A_cameraCarib = cv::Mat::zeros(calibPointsNum * 2, 8, CV_32F);
static cv::Mat caribPoints = (cv::Mat_<float>(calibPointsNum, 2) <<
			0.0112, 0.0112,
			0.46, 0.0112,
			0.46, 0.26,
			0.0112, 0.26);
...
//	1624, 930,
//	40, 930);
static cv::Mat cameraImgToDispTransform = cv::Mat::zeros(3, 3, CV_32F);

// Kinectの内部パラメータ
const static float f = 526.37013657;
const static float fx = 313.68782938;
const static float fy = 259.01834898;


static int capCount = 0;	// Kinectの座標変換行列を取得した回数
